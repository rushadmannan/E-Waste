...
else
{
  ...
  if(isset($_POST['id']))
  {
    if(!empty($_POST['id']))
    {
      ...
      $id=$_POST['id'];
      $name=$_POST['name'];
      $date_of_manu=date('d-m-Y',strtotime($_POST['date_of_manu']));
      $refurb_amount=$_POST['refurb_amount'];
      $repair_cost=$_POST['repair_cost'];
      $process_id=$_POST['process_id'];


      $sql="INSERT INTO refurbished(re_process_id,Product_name,date_of_manu,refurb_amount,repair_cost,process_id) VALUES(:id,:name,
      to_date('".$date_of_manu."','DD/MM/YYYY'),:refurb_amount,:repair_cost,:process_id)";
  ...
  oci_bind_by_name($compiled,':date_of_manu',$date_of_manu);
  oci_bind_by_name($compiled,':refurb_amount',$refurb_amount);
  oci_bind_by_name($compiled,':repair_cost',$repair_cost);
  ...
  header("Location:refurbished.php");
  exit;
}
}
}
...
<body>
  ...
  <main id="main">
    <div>
    <br><br>
    <h2 align='center'>Enter Refurbished Data: <h2>
    <form class="setInput" method="post">
        <label for="uname">RE PROCESS ID</label>
        ...
        <label for="uname">DATE OF MANU.</label>
        <input type="date" class="form-control" id="uname" placeholder="dd-mm-yyyy" name="date_of_manu" required>
        <label for="uname">REFURB AMOUNT</label>
        <input type="text" class="form-control" id="uname" placeholder="Enter Refurb Amount" name="refurb_amount" required>
        <label for="uname">REPAIR COST</label>
        <input type="text" class="form-control" id="uname" placeholder="Enter Repair Cost" name="repair_cost" required>
        <label for="uname">PROCESS ID</label>
        <input type="text" class="form-control" id="uname" placeholder="Enter Process Id" name="process_id" required>
